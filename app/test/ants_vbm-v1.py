...
from utils.scaleFactor import scaleFactor
...
def vbm(subject_label, session_label, target_template, age, patientSex, input, speed, HarvardOxford_Cortical, HarvardOxford_Subcortical, Glasser, Jolly, ICBM81):
    ...
    sf = scaleFactor(individualMaskedBrain)    
    ...
    if HarvardOxford_Subcortical == True:
        df, Backup_df = ROI.run_subcortical(FLYWHEEL_BASE, WORK, antsImageAlign, alignedBrainImage, gCorrectedGMSegmentation, GM_mask, brainAffineField, brainWarpField, brainInverseWarpField, df, Backup_df, sf)
        # individualMaskedBrain
 
    if HarvardOxford_Cortical == True:
        df, Backup_df = ROI.run_cortical(FLYWHEEL_BASE, WORK, antsImageAlign, alignedBrainImage, gCorrectedGMSegmentation, GM_mask, brainAffineField, brainWarpField, brainInverseWarpField, df, Backup_df, sf)
        # individualMaskedBrain
    if Glasser == True:
        print("Sorry, Glasser 2016 atlas is not yet implemented")
    
    if ICBM81 == True:
        df, Backup_df = ROI.run_ICBM81(FLYWHEEL_BASE, WORK, antsImageAlign, alignedBrainImage, gCorrectedWMSegmentation,  WM_mask, brainAffineField,brainWarpField, brainInverseWarpField, df, Backup_df, sf)
        # individualMaskedBrain
